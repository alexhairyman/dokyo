/* Converted to D from tcadb.h by htod */
module tcadb;
/*************************************************************************************************
...
//C     #ifndef _TCADB_H                         /* duplication check */
//C     #define _TCADB_H

//C     #if defined(__cplusplus)
//C     #define __TCADB_CLINKAGEBEGIN extern "C" {
//C     #define __TCADB_CLINKAGEEND }
//C     #else
//C     #define __TCADB_CLINKAGEBEGIN
//C     #define __TCADB_CLINKAGEEND
//C     #endif
//C     __TCADB_CLINKAGEBEGIN
...
//C     typedef struct {                         /* type of structure for an abstract database */
//C       int omode;                             /* open mode */
//C       TCMDB *mdb;                            /* on-memory hash database object */
//C       TCNDB *ndb;                            /* on-memory tree database object */
//C       TCHDB *hdb;                            /* hash database object */
//C       TCBDB *bdb;                            /* B+ tree database object */
//C       TCFDB *fdb;                            /* fixed-length databae object */
//C       TCTDB *tdb;                            /* table database object */
//C       int64_t capnum;                        /* capacity number of records */
//C       int64_t capsiz;                        /* capacity size of using memory */
//C       uint32_t capcnt;                       /* count for capacity check */
//C       BDBCUR *cur;                           /* cursor of B+ tree */
//C       void *skel;                            /* skeleton database */
//C     } TCADB;
struct _N46
{
    int omode;
...
//C       ADBOVOID,                              /* not opened */
//C       ADBOMDB,                               /* on-memory hash database */
//C       ADBONDB,                               /* on-memory tree database */
//C       ADBOHDB,                               /* hash database */
//C       ADBOBDB,                               /* B+ tree database */
//C       ADBOFDB,                               /* fixed-length database */
//C       ADBOTDB,                               /* table database */
//C       ADBOSKEL                               /* skeleton database */
//C     };
enum
{
    ADBOVOID,
...
//C     TCADB *tcadbnew(void);
TCADB * tcadbnew();


/* Delete an abstract database object.
   `adb' specifies the abstract database object. */
//C     void tcadbdel(TCADB *adb);
...
//C     bool tcadbopen(TCADB *adb, const char *name);
...
//C     bool tcadbput(TCADB *adb, const void *kbuf, int ksiz, const void *vbuf, int vsiz);
...
/* Store a string record into an abstract object.
   ...
   If successful, the return value is true, else, it is false.
   If a record with the same key exists in the database, it is overwritten. */
//C     bool tcadbput2(TCADB *adb, const char *kstr, const char *vstr);
...
//C     bool tcadbputkeep(TCADB *adb, const void *kbuf, int ksiz, const void *vbuf, int vsiz);
...
/* Store a new string record into an abstract database object.
   ...
   If successful, the return value is true, else, it is false.
   If a record with the same key exists in the database, this function has no effect. */
//C     bool tcadbputkeep2(TCADB *adb, const char *kstr, const char *vstr);
...
//C     bool tcadbputcat(TCADB *adb, const void *kbuf, int ksiz, const void *vbuf, int vsiz);
...
/* Concatenate a string value at the end of the existing record in an abstract database object.
   ...
   If successful, the return value is true, else, it is false.
   If there is no corresponding record, a new record is created. */
//C     bool tcadbputcat2(TCADB *adb, const char *kstr, const char *vstr);
...
/* Remove a record of an abstract database object.
   ...
   `ksiz' specifies the size of the region of the key.
   If successful, the return value is true, else, it is false. */
//C     bool tcadbout(TCADB *adb, const void *kbuf, int ksiz);
...
/* Remove a string record of an abstract database object.
   ...
   `kstr' specifies the string of the key.
   If successful, the return value is true, else, it is false. */
//C     bool tcadbout2(TCADB *adb, const char *kstr);
...
//C     void *tcadbget(TCADB *adb, const void *kbuf, int ksiz, int *sp);
...
//C     char *tcadbget2(TCADB *adb, const char *kstr);
...
/* Get the size of the value of a record in an abstract database object.
   ...
   it is -1. */
//C     int tcadbvsiz(TCADB *adb, const void *kbuf, int ksiz);
...
/* Get the size of the value of a string record in an abstract database object.
   ...
   If successful, the return value is the size of the value of the corresponding record, else,
   it is -1. */
//C     int tcadbvsiz2(TCADB *adb, const char *kstr);
...
//C     bool tcadbiterinit(TCADB *adb);
...
//C     void *tcadbiternext(TCADB *adb, int *sp);
...
/* Get the next key string of the iterator of an abstract database object.
   ...
   access. */
//C     char *tcadbiternext2(TCADB *adb);
...
//C     TCLIST *tcadbfwmkeys(TCADB *adb, const void *pbuf, int psiz, int max);
...
/* Get forward matching string keys in an abstract database object.
   ...
   deleted with the function `tclistdel' when it is no longer in use.  Note that this function
   may be very slow because every key in the database is scanned. */
//C     TCLIST *tcadbfwmkeys2(TCADB *adb, const char *pstr, int max);
...
//C     int tcadbaddint(TCADB *adb, const void *kbuf, int ksiz, int num);
...
//C     double tcadbadddouble(TCADB *adb, const void *kbuf, int ksiz, double num);
...
/* Optimize the storage of an abstract database object.
   ...
   successive updating. */
//C     bool tcadboptimize(TCADB *adb, const char *params);
...
/* Copy the database file of an abstract database object.
   ...
   the database file. */
//C     bool tcadbcopy(TCADB *adb, const char *path);
...
//C     bool tcadbtranbegin(TCADB *adb);
...
//C     bool tcadbtrancommit(TCADB *adb);
...
//C     bool tcadbtranabort(TCADB *adb);
...
/* Get the file path of an abstract database object.
   ...
   database. */
//C     const char *tcadbpath(TCADB *adb);
...
/* Get the number of records of an abstract database object.
   ...
   instance. */
//C     uint64_t tcadbrnum(TCADB *adb);
...
/* Get the size of the database of an abstract database object.
   ...
   database instance. */
//C     uint64_t tcadbsize(TCADB *adb);
...
//C     TCLIST *tcadbmisc(TCADB *adb, const char *name, const TCLIST *args);
...
//C     typedef struct {                         /* type of structure for a extra database skeleton */
//C       void *opq;                             /* opaque pointer */
//C       void (*del)(void *);                   /* destructor */
//C       bool (*open)(void *, const char *);
//C       bool (*close)(void *);
//C       bool (*put)(void *, const void *, int, const void *, int);
//C       bool (*putkeep)(void *, const void *, int, const void *, int);
//C       bool (*putcat)(void *, const void *, int, const void *, int);
//C       bool (*out)(void *, const void *, int);
//C       void *(*get)(void *, const void *, int, int *);
//C       int (*vsiz)(void *, const void *, int);
//C       bool (*iterinit)(void *);
//C       void *(*iternext)(void *, int *);
//C       TCLIST *(*fwmkeys)(void *, const void *, int, int);
//C       int (*addint)(void *, const void *, int, int);
//C       double (*adddouble)(void *, const void *, int, double);
//C       bool (*sync)(void *);
//C       bool (*optimize)(void *, const char *);
//C       bool (*vanish)(void *);
//C       bool (*copy)(void *, const char *);
//C       bool (*tranbegin)(void *);
//C       bool (*trancommit)(void *);
//C       bool (*tranabort)(void *);
//C       const char *(*path)(void *);
//C       uint64_t (*rnum)(void *);
//C       uint64_t (*size)(void *);
//C       TCLIST *(*misc)(void *, const char *, const TCLIST *);
//C       bool (*putproc)(void *, const void *, int, const void *, int, TCPDPROC, void *);
//C       bool (*foreach)(void *, TCITER, void *);
//C     } ADBSKEL;
struct _N48
{
    void *opq;
    void  function(void *)del;
    bool  function(void *, char *)open;
    bool  function(void *)close;
...
//    bool  function(void *, void *, int ) out;
    ...
    void * function(void *, int *)iternext;
    TCLIST * function(void *, void *, int , int )fwmkeys;
    int  function(void *, void *, int , int )addint;
    double  function(void *, void *, int , double )adddouble;
    bool  function(void *)sync;
    bool  function(void *, char *)optimize;
    bool  function(void *)vanish;
    bool  function(void *, char *)copy;
    bool  function(void *)tranbegin;
    bool  function(void *)trancommit;
    bool  function(void *)tranabort;
    char * function(void *)path;
    uint64_t  function(void *)rnum;
...
//C     typedef bool (*ADBMAPPROC)(void *map, const char *kbuf, int ksiz, const char *vbuf, int vsiz,
//C                                void *op);
alias bool  function(void *map, char *kbuf, int ksiz, char *vbuf, int vsiz, void *op)ADBMAPPROC;
...
/* Set an extra database sleleton to an abstract database object.
   ...
   If successful, the return value is true, else, it is false. */
//C     bool tcadbsetskel(TCADB *adb, ADBSKEL *skel);
...
/* Set the multiple database skeleton to an abstract database object.
   ...
   If successful, the return value is true, else, it is false. */
//C     bool tcadbsetskelmulti(TCADB *adb, int num);
...
//C     bool tcadbputproc(TCADB *adb, const void *kbuf, int ksiz, const void *vbuf, int vsiz,
...
/* Process each record atomically of an abstract database object.
   ...
   it is not needed, `NULL' can be specified.
   If successful, the return value is true, else, it is false. */
//C     bool tcadbforeach(TCADB *adb, TCITER iter, void *op);
...
/* Map records of an abstract database object into another B+ tree database.
   ...
   If successful, the return value is true, else, it is false. */
//C     bool tcadbmapbdb(TCADB *adb, TCLIST *keys, TCBDB *bdb, ADBMAPPROC proc, void *op, int64_t csiz);
...
/* Emit records generated by the mapping function into the result map.
   ...
   `vsiz' specifies the size of the region of the value.
   If successful, the return value is true, else, it is false. */
//C     bool tcadbmapbdbemit(void *map, const char *kbuf, int ksiz, const char *vbuf, int vsiz);
bool  tcadbmapbdbemit(void *map, char *kbuf, int ksiz, char *vbuf, int vsiz);



//C     __TCADB_CLINKAGEEND
